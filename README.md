...
### 6. `get_discovery_queue`
Discovery候補キューを取得します。他のユーザーが保存している人気記事の中から、AIがあなたの興味に合わせて選んだ記事を確認できます。
...
### 8. `dismiss_discovery_article`
Discovery候補から記事を「興味なし」として却下します。この記事は今後推薦されなくなります。

**パラメータ:**
- `item_id` (必須): Discovery キューアイテムのID（UUID形式）

**使用例:**
```
このDiscovery記事は興味ないので却下して（ID: xxx）
```

### 9. `import_articles`
複数の記事をCuraQに一括でインポートします。URLリストを指定すると、各記事を順次保存します。重複記事は自動的にスキップされ、部分的な失敗があっても残りの記事の処理を継続します。

**パラメータ:**
- `urls` (必須): インポートする記事のURLリスト（配列）
- `mark_as_read` (オプション): インポート後に記事を既読としてマークするか（デフォルト: `false`）
- `batch_size` (オプション): 一度に処理するバッチサイズ（デフォルト: 10、最大: 20）

**使用例:**
```
以下のURLリストを一括でインポートして:
- https://example.com/article1
- https://example.com/article2
- https://example.com/article3

このURLリストを既読記事としてインポートして:
- https://example.com/read1
- https://example.com/read2
```

**インポート結果:**
インポート完了後、以下の情報を含むサマリーが表示されます：
- 成功した記事数
- スキップ（重複）した記事数
- 失敗した記事数と理由
